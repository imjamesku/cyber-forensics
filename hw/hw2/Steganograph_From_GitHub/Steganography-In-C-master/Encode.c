...
void stega_encrypt(FILE *fp1, FILE *fp2, FILE *fp3) {
  char file_buff = 0, msg_buff = 0,
       ch; // temp var for one byte from file and msg
  int i;
  int bit_msg;
...
int Encoding(char *image, char *secretPath, char *destPath) {
  // Image, secret txt, destination
  FILE *fp1, *fp2, *fp3;

  // opening Image File
  if ((fp1 = fopen(image, "r+")) == NULL) {
